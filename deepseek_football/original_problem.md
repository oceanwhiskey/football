...
Players `i` and `j` (`1 <= i < j <= n`) on the same team want to switch to the other team. The other players then vote on whether this transfer should take place. For each `1 <= x <= n`:
...
Currently the teams are represented by the string `p`, where `p_i` is the team of the `i`-th player. Each player would really like to be in team `q_i`. Is it possible for some sequence of transfers to occur such that at the end, the team of the `i`-th player is `q_i` for all `1 <= i <= n`?
...
It is guaranteed that the sum of `n` over all test cases does not exceed `2 * 10^5`, and that each team has an odd number of players initially (so as a result, each team has at least `1` player, and `n` is always even).

...
\begin{defn}
Es sei $(n, p, q)$ wie in Definition \ref{defn:problembeschreibung} gegeben. Wir nennen $(n, p, q)$ \emph{zentrumsgleich}, wenn $p_{\frac{n}{2}}=q_{\frac{n}{2}}$ und $p_{\frac{n}{2}+1}=q_{\frac{n}{2}+1}$ gilt.
\label{defn:zentrumsgleich}
\end{defn}

\begin{cor}
Es sei $(n, p, q)$ wie in Definition \ref{defn:problembeschreibung} gegeben. Ist $(n, p, q)$ lösbar, so ist es zentrumsgleich.
\label{cor:zentrum invariant}
\end{cor}
...
\begin{thm}
Es sei $(n, p, q)$ wie in Definition \ref{defn:problembeschreibung} gegeben. Genau dann ist $(n, p, q)$ lösbar, wenn es zentrumsgleich, ausbalanciert und tauschbar ist.
\begin{proof}
Dass ein lösbares $(n,p,q)$ zentrumsgleich sein muss, folgt aus Korollar \ref{cor:zentrum invariant}. Dass ein lösbares $p$ auch ausbalanciert sein muss, folgt aus Lemma \ref{lem_ausbalanciert}. Dass ein lösbares $p$ zudem impliziert, dass $(n, p, q)$ tauschbar ist, ist mit Korollar \ref{cor:einmal tauschbar immer tauschbar} auch klar. Für die umgekehrte Richtung setzen wir voraus, dass $(n,p,q)$ zentrumsgleich, ausbalanciert und tauschbar ist, und nehmen an, das Problem wäre dennoch nicht lösbar. Dann können wir solange Wechsel ausführen bis wir eine kleinste Anzahl an nicht erfüllbaren Wechseln erreichen. Wir betrachten das zugehörige $\bar{p}\sim p$. Dann ist also $\sigma(n, \bar{p}, q)>0$. Außerdem ist wegen $\bar{p}\sim p$ mit Korollar \ref{cor:einmal tauschbar immer tauschbar} auch $(n, \bar{p}, q)$ tauschbar.
Da $(n,p,q)$ zentrumsgleich ist, gilt $p_{\frac{n}{2}}=q_{\frac{n}{2}}$ und $p_{\frac{n}{2}+1}=q_{\frac{n}{2}+1}$, und mit Korollar \ref{cor:zentrum invariant} angewandt auf $\bar p\sim p$ gilt auch $\bar p_{\frac{n}{2}}=q_{\frac{n}{2}}$ und $\bar p_{\frac{n}{2}+1}=q_{\frac{n}{2}+1}$. Somit kann $\sigma(n,\bar p, q)>0$ nur von einem Index aus $L\cup R$ herrühren, d.h. es gibt $i\in L$ mit $\bar p_i\neq q_i$ oder $j\in R$ mit $\bar p_j\neq q_j$ -- genau die Voraussetzung, die Lemma \ref{lem:tausch auf beiden seiten} benötigt.
...
\begin{proof}
Nach Satz \ref{thm} müssen wir nur prüfen, ob $(n, p, q)$ zentrumsgleich, ausbalanciert und tauschbar ist.
Ob $(n,p,q)$ zentrumsgleich ist, lässt sich direkt anhand der beiden Werte $p_{\frac{n}{2}}, q_{\frac{n}{2}}$ und $p_{\frac{n}{2}+1}, q_{\frac{n}{2}+1}$ in $O(1)$ prüfen. Die Bilanzen lassen sich in $O(n)$ berechnen. Anschließend muss man mit Proposition \ref{prop:i tauschbar => i'<i tauschbar} nur prüfen, ob $\max_{i\in L, p_i\neq q_i} i$ mit $n$ tauschbar ist und  ob $\min_{j\in R, p_j\neq q_j} j$ mit $1$ tauschbar ist. Das ist jeweils in $O(n)$ Aufwand mit $O(1)$ Speicher möglich.
\end{proof}
\end{cor}
